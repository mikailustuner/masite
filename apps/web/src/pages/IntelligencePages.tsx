        ...
        {sites.map((site) => (
          <article key={site.id} className="surface-card site-detail-card">
            <div className="site-card-top">
              ...
              <button className="more-button" aria-label={`${site.name} seçenekleri`}><MoreHorizontal size={18} /></button>
            </div>
...
export function VisibilityPage({ activeSite }: { activeSite: SiteSummary }) {
  const [series,setSeries]=useState<VisibilitySnapshot[]>([]); useEffect(()=>{void api.visibility(activeSite.id).then(setSeries);},[activeSite.id]);
  const values=series.map((item)=>item.score); const minimum=Math.min(...values,0); const maximum=Math.max(...values,100); const chartPoints=values.length>1?values.map((value,index)=>`${(index/(values.length-1))*720},${210-((value-minimum)/Math.max(1,maximum-minimum))*180}`).join(" "):""; const current=series.at(-1);
  return (
    <div className="page-stack">
      <section className="page-intro"><div><span className="section-kicker">Organik arama</span><h2>Search visibility</h2><p>{activeSite.name} için konum, cihaz ve SERP özelliği ağırlıklı görünürlük.</p></div><button className="secondary-button"><Download size={16} /> Dışa aktar</button></section>
      <section className="visibility-summary-grid">
        <article className="surface-card visibility-hero-card">
          <div className="card-heading"><div><span className="section-kicker">Son 12 hafta</span><h3>Görünürlük trendi</h3></div><div className="chart-filter"><button className="is-active">Visibility</button><button>Top 10</button><button>Top 3</button></div></div>
          <div className="visibility-number"><strong>{current ? `${current.score}%` : "Ölçülmedi"}</strong>{current && <span className="metric-change positive"><CircleDot size={14} /> {current.measured}/{current.tracked} keyword</span>}</div>
          <div className="main-line-chart">
            <div className="chart-y-labels"><span>75</span><span>60</span><span>45</span><span>30</span></div>
      ...
      <section className="surface-card topic-table-card">
        <div className="card-heading"><div><span className="section-kicker">Konu performansı</span><h3>Keyword kümeleri</h3></div><button className="text-button">Tüm kümeler <ChevronRight size={15}/></button></div>
...
export function KeywordsPage({ activeSite }: { activeSite: SiteSummary }) {
  ...
  const filtered = keywords.filter((keyword) => keyword.term.toLocaleLowerCase("tr").includes(query.toLocaleLowerCase("tr")));
  return (
    <div className="page-stack">
      ...
      <section className="surface-card data-table-card keyword-table-card">
        <div className="table-toolbar"><label className="table-search"><Search size={17}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Keyword ara" /></label><button className="secondary-button compact"><ListFilter size={16}/> Filtrele</button></div>
  ...
  return (
    <div className="page-stack">
      ...
      <section className="surface-card competitor-matrix">
        <div className="card-heading"><div><span className="section-kicker">Keyword overlap</span><h3>Rekabet haritası</h3></div><span className="data-source-note"><CircleDot size={13}/> Son ölçüm bugün</span></div>
        <div className="competitor-plot" aria-label="Rakip görünürlük ve keyword örtüşme grafiği">
          ...
          <div className="plot-bubble own" style={{left:"59%",top:"41%"}}><span>N</span><strong>{activeSite.name}</strong></div>
      ...
      <section className="competitor-card-grid">
        {competitors.map((competitor, index) => <article className="surface-card competitor-card" key={competitor.id}>
          <div className="competitor-card-head"><div className={`competitor-avatar c${index + 1}`}>{competitor.name.charAt(0)}</div><div><h3>{competitor.name}</h3><span>{new URL(competitor.origin).hostname}</span></div><button className="more-button" aria-label="Rakibi sil" onClick={() => api.deleteCompetitor(competitor.id).then(load)}><MoreHorizontal size={18}/></button></div>
          ...
          <button className="full-text-button">Kanıtlı karşılaştırmayı aç <ArrowRight size={15}/></button>
...
export function AdsStudioPage({ activeSite }: { activeSite: SiteSummary }) {
  ...
  const create = async () => { await api.createAdBrief({ siteId: activeSite.id, platform, ...form }); setCreating(false); await load(); };
  ...
  return (
    <div className="page-stack">
      ...
      {creating && <section className="surface-card brief-form"><div className="chart-filter"><button className={platform === "google" ? "is-active" : ""} onClick={() => setPlatform("google")}>Google</button><button className={platform === "meta" ? "is-active" : ""} onClick={() => setPlatform("meta")}>Meta</button></div><input placeholder="Brief adı" value={form.name} onChange={(e) => setForm({...form,name:e.target.value})}/><input placeholder="Hedef" value={form.objective} onChange={(e) => setForm({...form,objective:e.target.value})}/><input placeholder="Kitle" value={form.audience} onChange={(e) => setForm({...form,audience:e.target.value})}/><textarea placeholder="Teklif" value={form.offer} onChange={(e) => setForm({...form,offer:e.target.value})}/><button className="primary-button" onClick={() => void create()}>Kanıtlı taslak üret</button></section>}
      <section className="studio-platforms">
        <article className="surface-card platform-card google-card"><div className="platform-icon"><Search size={23}/></div><div><span>Google Ads Studio</span><h3>Arama talebini kampanya yapısına dönüştür</h3><p>Ad group, RSA, negatif keyword, asset ve landing page eşleştirmeleri.</p></div><div className="platform-stats"><span><strong>8</strong> taslak kampanya</span><span><strong>24</strong> test hipotezi</span></div><button>Studio’yu aç <ArrowRight size={16}/></button></article>
        <article className="surface-card platform-card meta-card"><div className="platform-icon"><Megaphone size={23}/></div><div><span>Meta Ads Studio</span><h3>Kanıtlı açılardan kreatif sistemleri kur</h3><p>Hook, vaat, itiraz, format ve CTA matrisleriyle kontrollü üretim.</p></div><div className="platform-stats"><span><strong>12</strong> kreatif brief</span><span><strong>36</strong> varyant</span></div><button>Studio’yu aç <ArrowRight size={16}/></button></article>
      </section>
      <section className="studio-workspace">
        ...
        <aside className="surface-card creative-preview"><div className="preview-toolbar"><span>Üretilen JSON</span><div><button className="is-active"><MonitorSmartphone size={15}/></button><button><Image size={15}/></button><button><Play size={15}/></button></div></div><div className="ad-preview"><small>{latest ? `${latest.platform.toUpperCase()} taslağı · ${activeSite.domain}` : "Taslak yok"}</small><h3>{latest?.offer ?? "Yeni bir brief oluşturun"}</h3><p>{latest ? JSON.stringify(latest.content, null, 2).slice(0, 420) : "Kampanya önerileri burada görünür."}</p></div><button className="secondary-button" disabled={!latest}><Download size={16}/> JSON dışa aktar</button></aside>
      </section>
    </div>
  );
}
...
export function ReportsPage({ activeSite }: { activeSite: SiteSummary }) {
  ...
  const share=async(reportId:string)=>{const result=await api.shareReport(reportId);await navigator.clipboard.writeText(result.url);setNotice(`14 gün geçerli bağlantı kopyalandı.`);};
